        ...
        <div class="flex justify-between h-16">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md focus:outline-none transition ease-in-out duration-150" style="color:white;background:rgba(255,255,255,0.12);border-radius:8px;">
                            <div style="color:white;font-weight:700;">{{ Auth::user()->name }}</div>
                            <div class="ms-1">
                                <svg class="fill-current h-4 w-4" style="fill:white;" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                    ...
                    <x-slot name="content">
                        @if(auth()->user()->isEstudiante())
                            <x-dropdown-link href="{{ route('estudiante.perfil') }}">
                                {{ __('Mi perfil') }}
                            </x-dropdown-link>
                        @endif
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            ...
            <div class="mt-3 space-y-1">
                @if(auth()->user()->isEstudiante())
                    <x-responsive-nav-link href="{{ route('estudiante.perfil') }}">
                        {{ __('Mi perfil') }}
                    </x-responsive-nav-link>
                @endif
